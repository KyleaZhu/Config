const CFG = { id: '6a2fa0be-0c92-4ccf-8c8d-bf81342754e1', chunk: 64 * 1024, dnPack: 32 * 1024, dnTail: 512, dnMs: 0, upPack: 16 * 1024, upQMax: 256 * 1024, maxED: 8 * 1024, concur: 4 };
export default { fetch: async (req, env) => req.headers.get('Upgrade')?.toLowerCase() === 'websocket' ? ws(req, env) : new URL(req.url).pathname !== '/' + env.KEY ? new Response('🚧 此路不通，请绕行～', { status: 404 }) : new Response((await fetch(env.ADDRESS).then(r => r.text())).split('\n').filter(l => l.trim()).map(a => { const [x, n = env.NAME] = a.split('#'); return `vless://${CFG.id}@${x}?path=%2F%3Fed%3D2560&security=tls&encryption=none&insecure=0&host=${env.DOMAIN}&fp=firefox&ech=cloudflare-ech.com%2Bhttps%3A%2F%2F223.5.5.5%2Fdns-query&type=ws&allowInsecure=0&sni=${env.DOMAIN}#${encodeURIComponent(n)}`; }).join('\n')) }; const hex = c => (c > 64 ? c + 9 : c) & 0xF;
...
const ws = async (req, env) => {
  ...
  const thresh = async () => { if (busy || closed) return; busy = true; try { for (;;) {
    if (closed) break; if (!sock) { const [d] = uq.bundle(); if (!d) break; const r = vless(d); if (!r) throw wither(); server.send(new Uint8Array([d[0], 0])); const host = addr(r.addrType, r.targetAddrBytes), port = r.port, payload = d.subarray(r.dataOffset); try { sock = await raceSprout(fetcher, host, port); } catch { sock = await raceSprout(fetcher, env.PROXYIP, 443); } if (!sock) throw wither(); curW = sock.writable.getWriter(); const [first] = uq.bundle(payload); first?.byteLength && await curW.write(first); mill(sock.readable, server).finally(() => wither()); continue; }
