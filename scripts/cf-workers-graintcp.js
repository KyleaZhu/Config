...
export default { fetch: async (req, env) => req.headers.get('Upgrade')?.toLowerCase() === 'websocket' ? ws(req, env) : new URL(req.url).pathname !== '/' + env.KEY ? new Response('🚧 此路不通，请绕行～', { status: 404 }) : new Response((await fetch(env.ADDRESS).then(r => r.text())).split('\n').filter(l => l.trim()).map(a => { const [x, n] = a.split('#'); const [h, p = 443] = x.split(':'); return `vless://${CFG.id}@${h}:${p}?path=%2F%3Fed%3D2560&security=tls&encryption=none&insecure=0&host=${env.DOMAIN}&fp=firefox&type=ws&allowInsecure=0&sni=${env.DOMAIN}#${encodeURIComponent(n || env.NAME)}`; }).join('\n')) }; const hex = c => (c > 64 ? c + 9 : c) & 0xF;
