const CFG = { chunk: 64 * 1024, dnPack: 32 * 1024, dnTail: 512, dnMs: 0, upPack: 16 * 1024, upQMax: 256 * 1024, maxED: 8 * 1024, concur: 4 };
export default { fetch: async (req, env) => req.headers.get('Upgrade')?.toLowerCase() === 'websocket' ? ws(req, env) : new Response((await fetch(env.ADDRESS).then(r => r.text())).split(/[\s,]+/).filter(Boolean).map(a => { const [x, n] = a.split('#'); const [h, p = 443] = x.split(':'); return `vless://${env.UUID}@${h}:${p}?path=%2F%3Fed%3D2560&security=tls&host=${env.DOMAIN}&fp=firefox${env.ECH == '1' ? '&ech=cloudflare-ech.com%2Bhttps%3A%2F%2F223.5.5.5%2Fdns-query' : ''}&type=ws&sni=${env.DOMAIN}#${encodeURIComponent(n || env.NAME)}`; }).join('\n')) }; const hex = c => (c > 64 ? c + 9 : c) & 0xF;
const dec = new TextDecoder(), idBytes = id => { const b = new Uint8Array(16); for (let i = 0, p = 0, c, h; i < 16; i++) { c = id.charCodeAt(p++); c === 45 && (c = id.charCodeAt(p++)); h = hex(c); c = id.charCodeAt(p++); c === 45 && (c = id.charCodeAt(p++)); b[i] = h << 4 | hex(c); } return b; };
const matchID = (c, b) => c[1] === b[0] && c[2] === b[1] && c[3] === b[2] && c[4] === b[3] && c[5] === b[4] && c[6] === b[5] && c[7] === b[6] && c[8] === b[7] && c[9] === b[8] && c[10] === b[9] && c[11] === b[10] && c[12] === b[11] && c[13] === b[12] && c[14] === b[13] && c[15] === b[14] && c[16] === b[15];
...
const vless = (c, b) => { if (c.length < 24 || !matchID(c, b)) return null; let o = 19 + c[17]; const p = (c[o] << 8) | c[o + 1]; let t = c[o + 2]; if (t !== 1) t += 1; const a = parseAddr(c, o + 3, t); return a ? { addrType: t, ...a, port: p } : null; };
...
const ws = async (req, env) => {
  ...
  const thresh = async () => { if (busy || closed) return; busy = true; try { for (;;) {
    if (closed) break; if (!sock) { const [d] = uq.bundle(); if (!d) break; const r = vless(d, idB); if (!r) throw wither(); server.send(new Uint8Array([d[0], 0])); const host = addr(r.addrType, r.targetAddrBytes), port = r.port, payload = d.subarray(r.dataOffset); try { sock = await raceSprout(fetcher, host, port); } catch { sock = await raceSprout(fetcher, env.PROXYIP, 443); } if (!sock) throw wither(); curW = sock.writable.getWriter(); const [first] = uq.bundle(payload); first?.byteLength && await curW.write(first); mill(sock.readable, server).finally(() => wither()); continue; }
