...
export default { fetch: async (req, env) => req.headers.get('Upgrade')?.toLowerCase() === 'websocket' ? ws(req, env) : new URL(req.url).pathname !== '/' + env.KEY ? new Response('🚧 此路不通，请绕行～', { status: 404 }) : new Response((await fetch(env.ADDRESS).then(r => r.text())).split('\n').filter(l => l.trim()).map(a => { const [x, n = env.NAME] = a.split('#'); return `vless://${CFG.id}@${x}?path=%2F${env.PROXYIP}%3Fed%3D2560&security=tls&encryption=none&insecure=0&host=${env.DOMAIN}&fp=firefox&ech=cloudflare-ech.com%2Bhttps%3A%2F%2F223.5.5.5%2Fdns-query&type=ws&allowInsecure=0&sni=${env.DOMAIN}#${encodeURIComponent(n)}`; }).join('\n')) }; const hex = c => (c > 64 ? c + 9 : c) & 0xF;
