const CFG = { id: '99be346e-d94a-402b-b498-d0b7fc089f73', chunk: 64 * 1024, dnPack: 32 * 1024, dnTail: 512, dnMs: 0, upPack: 16 * 1024, upQMax: 256 * 1024, maxED: 8 * 1024, concur: 4 };
export default { fetch: async (req, env) => req.headers.get('Upgrade')?.toLowerCase() === 'websocket' ? ws(req, env) : new Response((await fetch(env.ADDRESS).then(r => r.text())).split(/[\s,]+/).filter(Boolean).map(a => { const [x, n] = a.split('#'); const [h, p = 443] = x.split(':'); return `vless://${CFG.id}@${h}:${p}?path=%2F%3Fed%3D2560&security=tls&host=${env.DOMAIN}&fp=firefox&ech=cloudflare-ech.com%2Bhttps%3A%2F%2F223.5.5.5%2Fdns-query&type=ws&sni=${env.DOMAIN}#${encodeURIComponent(n || env.NAME)}`; }).join('\n')) }; const hex = c => (c > 64 ? c + 9 : c) & 0xF;
