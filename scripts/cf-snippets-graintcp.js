const CFG = { id: '6d760851-165d-45e8-8464-d4a2578fe9cd', chunk: 64 * 1024, dnPack: 32 * 1024, dnTail: 512, dnQr: 4, upPack: 20 * 1024, maxED: 8 * 1024, concur: 1, key: 'fuckseu', address: 'https://raw.githubusercontent.com/KyleaZhu/Config/main/bestcf/ip.txt', name: 'GrainTCP', domain: 'sni.abe.qzz.io', proxyip: 'ProxyIP.US.CMLiussss.net' };
export default { fetch: async req => req.headers.get('Upgrade')?.toLowerCase() === 'websocket' ? ws(req) : new URL(req.url).pathname !== '/' + CFG.key ? new Response('🚧 此路不通，请绕行～', { status: 404 }) : new Response((await fetch(CFG.address).then(r => r.text())).split('\n').filter(l => l.trim()).map(a => { const [x, n = CFG.name] = a.split('#'); return `vless://${CFG.id}@${x}?path=%2F${CFG.proxyip}%3Fed%3D2560&security=tls&encryption=none&insecure=0&host=${CFG.domain}&fp=firefox&ech=cloudflare-ech.com%2Bhttps%3A%2F%2F223.5.5.5%2Fdns-query&type=ws&allowInsecure=0&sni=${CFG.domain}#${encodeURIComponent(n)}`; }).join('\n')) }; const hex = c => (c > 64 ? c + 9 : c) & 0xF;
