...
function getPoolFixedName(poolKey) {
    ...
    if (poolKey === POOL_TRASH_KEY) return '垃圾桶';
...
// 批量调用检测接口，并统一整理 API 返回的出口、ASN、国家或地区信息
...
function getTargetFilterSummary(target) {
    return [target.country ? `国家或地区:${target.country}` : '', target.asn ? `ASN:${target.asn}` : ''].filter(Boolean).join(', ') || '无';
}
...
async function runMaintenanceCore({
    ...
    for (const { item, result } of await checkCurrentItems(currentItems, checkFn)) {
        ...
        const reason = result.success ? '出口/国家或地区/ASN不匹配' : '检测失效';
        ...
        trashBatch.push({ ipAddr: removed.entry || item.addr, reason: result.success ? '出口/国家或地区/ASN不匹配' : '维护失效', poolKey });
        addLog(formatCurrentRemovedLog
            ? formatCurrentRemovedLog(item, result)
            : (result.success ? `  ❌ ${item.addr} - 出口/国家或地区/ASN不匹配，已移除` : `  ❌ ${item.addr} - 失效已移除，已放入垃圾桶`));
    }
        ...
        for (const item of candidates) {
            ...
            if (!checkResultMatchesTarget(result, target)) {
                if (result.success) {
                    ...
                    addLog(`  ⏭️ ${candidate.addr} - 出口/国家或地区/ASN不匹配`);
                } else {
...
async function maintainTXTRecords(env, target, addLog, report, poolKey, checkFn, config) {
    ...
    const state = await runMaintenanceCore({
        ...
        formatCurrentRemovedLog: (item, result) => result.success
            ? `  ❌ ${item.addr} - 出口/国家或地区/ASN不匹配，已从TXT移除`
                ...
                <div class="ip-content-area">
                    ...
                    <div class="mb-2 filter-toolbar">
                        ...
                        <div id="filter-help" class="filter-help" hidden>
                            支持空格分隔条件：<code>port:443</code>、<code>port:443-2053</code>、<code>country:国家或地区代码</code>、<code>asn:ASN编号</code>、普通关键词。空格表示“且”，竖线 <code>|</code> 表示“或”,例如 <code>country:KR asn:AS4766 | country:US</code>
                        </div>
...
<script>
    ...
    // 池名显示（统一格式）
    const POOL_NAMES = { [POOL_DEFAULT_KEY]: '默认池', [POOL_TRASH_KEY]: '垃圾桶' };
    const NUMBERED_POOL_KEY_RE = /^ip_pool_(\\d{3})$/;
    ...
    function buildTargetCard(target = {}, index = 0) {
        ...
        const filters = [target.country ? '国家或地区 ' + target.country : '', target.asn ? 'ASN ' + target.asn : ''].filter(Boolean).join(' · ');
        const enabled = target.enabled !== false;
        return \`
    ...
    function showTargetEditor(index, target = {}) {
        ...
        const targetFields = [
            ...
            renderField('input', { id: 'edit-target-country', label: '国家或地区', hint: '可选，填写国家或地区代码。', value: target.country || '', placeholder: '国家或地区代码' }),
